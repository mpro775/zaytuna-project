...
/**
 * سكريبت اختبار وحدة الإشعارات (Notification Module)
 * يمكن تشغيله بـ: npm run notification:test
 */
...
async function testNotificationSystem() {
  ...
  try {
    ...
    // ===== اختبار 4: إرسال إشعار باستخدام قالب =====
    console.log('\n📋 إرسال إشعار باستخدام قالب...');

    if (testTemplateId) {
      const templatedResponse = await request(server)
    ...
    // ===== اختبار 6: الحصول على الأحداث المتاحة =====
    ...
    // ===== اختبار 7: تفضيلات الإشعارات =====
    console.log('\n⚙️ اختبار تفضيلات الإشعارات...');

    // الحصول على التفضيلات الافتراضية
    const defaultPrefsResponse = await request(server)
    ...
    // إحصائيات التفضيلات
    const prefsStatsResponse = await request(server)
    ...
    // ===== اختبار 10: معلومات مزودي الخدمة =====
    console.log('\n🔧 معلومات مزودي الخدمة...');

    const providersResponse = await request(server)
    ...
    // ===== اختبار 11: إنشاء القوالب الافتراضية =====
    console.log('\n📝 إنشاء القوالب الافتراضية...');

    const defaultTemplatesResponse = await request(server)
