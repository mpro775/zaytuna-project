...
/**
 * سكريبت اختبار وحدة التقارير (Reporting Module)
 * يمكن تشغيله بـ: npm run reporting:test
 */
...
async function testReportingSystem() {
  ...
  try {
    ...
    const salesReportResponse = await request(server)
    ...
    if (salesReportResponse.status === 200 && salesReportResponse.body) {
      ...
      if (salesReportResponse.body.byPeriod && salesReportResponse.body.byPeriod.length > 0) {
        console.log('Revenue by Period (last 3):',
          salesReportResponse.body.byPeriod.slice(-3).map(p => `${p.period}: ${p.revenue}`)
        );
      }

      if (salesReportResponse.body.byPaymentMethod && salesReportResponse.body.byPaymentMethod.length > 0) {
        console.log('Payment Methods:',
          salesReportResponse.body.byPaymentMethod.map(pm => `${pm.method}: ${pm.percentage.toFixed(1)}%`)
        );
      }
    } else {
    ...
    // ===== اختبار 3: تقرير المخزون =====
    console.log('\n📦 اختبار تقرير المخزون...');

    const inventoryReportResponse = await request(server)
    ...
    if (inventoryReportResponse.status === 200 && inventoryReportResponse.body) {
      ...
      if (inventoryReportResponse.body.lowStockAlerts && inventoryReportResponse.body.lowStockAlerts.length > 0) {
        console.log('Low Stock Alerts (first 3):',
          inventoryReportResponse.body.lowStockAlerts.slice(0, 3).map(alert =>
            `${alert.productName}: ${alert.currentStock}/${alert.minStock} (${alert.warehouseName})`
          )
        );
      }

      if (inventoryReportResponse.body.topMovingProducts && inventoryReportResponse.body.topMovingProducts.length > 0) {
        console.log('Top Moving Products (first 3):',
          inventoryReportResponse.body.topMovingProducts.slice(0, 3).map(product =>
            `${product.productName}: ${product.totalOut} out, ${product.currentStock} in stock`
          )
        );
      }
    } else {
    ...
    if (dashboardResponse.status === 200 && dashboardResponse.body) {
      ...
      if (dashboardResponse.body.charts?.revenueByPeriod && dashboardResponse.body.charts.revenueByPeriod.length > 0) {
        console.log('Revenue Trend (last 3 months):',
          dashboardResponse.body.charts.revenueByPeriod.slice(-3).map(p => `${p.period}: ${p.revenue}`)
        );
      }

      if (dashboardResponse.body.charts?.topProducts && dashboardResponse.body.charts.topProducts.length > 0) {
        console.log('Top Products (first 3):',
          dashboardResponse.body.charts.topProducts.slice(0, 3).map(p => `${p.productName}: ${p.revenue}`)
        );
      }
    ...
    // ===== اختبار 5: تقرير المبيعات الشهري =====
    console.log('\n📅 اختبار تقرير المبيعات الشهري...');

    const currentDate = new Date();
    const monthlyReportResponse = await request(server)
    ...
    // ===== اختبار 6: تقرير المخزون المنخفض =====
    ...
    if (lowStockResponse.status === 200 && lowStockResponse.body) {
      ...
      if (lowStockResponse.body.lowStockAlerts && lowStockResponse.body.lowStockAlerts.length > 0) {
        console.log('Low Stock Items (first 3):',
          lowStockResponse.body.lowStockAlerts.slice(0, 3).map(item =>
            `${item.productName}: ${item.currentStock}/${item.minStock} (${item.warehouseName})`
          )
        );
      }
    } else {
    ...
    // ===== اختبار 7: إحصائيات المحاسبة =====
    console.log('\n🧮 اختبار إحصائيات المحاسبة...');

    const accountingStatsResponse = await request(server)
    ...
    // ===== اختبار 8: تقارير بصيغ مختلفة =====
    console.log('\n📄 اختبار تصدير التقارير...');

    try {
      const excelExportResponse = await request(server)
    ...
    try {
      const pdfExportResponse = await request(server)
