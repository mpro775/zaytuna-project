...
/**
 * سكريبت اختبار نظام المشتريات (Purchasing System)
 * يمكن تشغيله بـ: npm run purchasing:test
 */
...
async function testPurchasingSystem() {
  ...
  let testPurchaseInvoiceId: string = '';
  ...
  try {
    ...
    if (productsResponse.status === 200 && productsResponse.body.length > 0) {
      ...
      const productResponse = await request(server)
        ...
        .send({
          ...
          categoryId: 'some-category-id', // سيتم إصلاحه لاحقاً
          basePrice: 199.99,
          costPrice: 150.00,
          trackInventory: true,
          reorderPoint: 5,
    ...
    // ===== اختبار 4: الحصول على متغيرات المنتج =====
    console.log('\n🎨 الحصول على متغيرات المنتج...');

    const variantsResponse = await request(server)
    ...
    const orderResponse = await request(server)
      ...
      .send({
        ...
        expectedDate: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // بعد أسبوع
        lines: [
          {
            productId: testProductId,
            quantity: 10,
    ...
    // ===== اختبار 8: تحديث حالة أمر الشراء =====
    console.log('\n📝 تحديث حالة أمر الشراء...');

    const updateOrderResponse = await request(server)
    ...
    const invoiceResponse = await request(server)
      ...
      .send({
        ...
        dueDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // بعد 30 يوم
        lines: [
          {
    ...
    // ===== اختبار 10: إنشاء دفعة لفاتورة الشراء =====
    ...
    // ===== اختبار 11: الحصول على الموردين =====
    console.log('\n🏢 الحصول على الموردين...');

    const suppliersResponse = await request(server)
    ...
    // ===== اختبار 12: الحصول على أوامر الشراء =====
    ...
    // ===== اختبار 14: التحقق من تحديث المخزون =====
