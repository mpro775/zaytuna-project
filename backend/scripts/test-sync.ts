...
/**
 * سكريبت اختبار وحدة المزامنة (Sync Module)
 * يمكن تشغيله بـ: npm run sync:test
 */
  ...
  try {
    ...
    if (createUserResponse.status === 201) {
      ...
      if (usersResponse.status === 200) {
        const testUser = usersResponse.body.data.find((u: any) => u.username === 'sync_test_user');
        if (testUser) {
    ...
    // ===== اختبار 3: إنشاء دفعة مزامنة =====
    console.log('\n📦 إنشاء دفعة مزامنة...');

    const createBatchResponse = await request(server)
      .post('/sync/upload')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({
    ...
    // ===== اختبار 6: التحقق من صحة جلسة offline =====
    ...
    // ===== اختبار 7: جلب حزمة البيانات للعمل offline =====
    ...
    // ===== اختبار 8: حفظ تغييرات من وضع offline =====
    console.log('\n💾 حفظ تغييرات من وضع offline...');

    if (testSessionId) {
      const saveChangesResponse = await request(server)
    ...
    // ===== اختبار 9: جلسات الجهاز في وضع offline =====
    console.log('\n📱 جلسات الجهاز في وضع offline...');

    const deviceSessionsResponse = await request(server)
    ...
    // ===== اختبار 10: إحصائيات وضع offline =====
    console.log('\n📊 إحصائيات وضع offline...');

    const offlineStatsResponse = await request(server)
    ...
    // ===== اختبار 11: تنظيف الجلسات المنتهية =====
    ...
    // ===== اختبار 12: إنهاء جلسة offline =====
    console.log('\n🏁 إنهاء جلسة offline...');

    if (testSessionId) {
      const endSessionResponse = await request(server)
    ...
    // ===== اختبار 13: تنظيف دفعات المزامنة القديمة =====
    console.log('\n🗑️ تنظيف دفعات المزامنة القديمة...');

    const cleanupBatchesResponse = await request(server)
