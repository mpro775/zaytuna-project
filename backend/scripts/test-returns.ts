...
/**
 * سكريبت اختبار نظام المرتجعات (Returns System)
 * يمكن تشغيله بـ: npm run returns:test
 */
...
async function testReturnsSystem() {
  ...
  try {
    ...
    // ===== اختبار 6: الحصول على متغيرات المنتج =====
    ...
    const invoiceResponse = await request(server)
      ...
      .send({
        ...
        branchId: 'some-branch-id', // سيتم إصلاحه
    ...
    // ===== اختبار 10: الحصول على المرتجعات =====
    console.log('\n📋 الحصول على المرتجعات...');

    const returnsResponse = await request(server)
      .get('/returns')
      .set('Authorization', `Bearer ${adminToken}`);
    ...
    // ===== اختبار 11: إنشاء إشعار دائن =====
    console.log('\n📄 إنشاء إشعار دائن...');

    const creditNoteResponse = await request(server)
    ...
    // ===== اختبار 13: التحقق من تحديث المخزون =====
