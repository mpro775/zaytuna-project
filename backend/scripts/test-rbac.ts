...
/**
 * سكريبت اختبار نظام إدارة الأدوار والصلاحيات (RBAC)
 * يمكن تشغيله بـ: npm run rbac:test
 */
...
async function testRBACSystem() {
  ...
  try {
    ...
    // ===== اختبار 2: الحصول على جميع الصلاحيات =====
    ...
    const createRoleResponse = await request(server)
      .post('/roles')
      .set('Authorization', `Bearer ${adminToken}`)
      .send(newRole);
    ...
    // ===== اختبار 4: الحصول على جميع الأدوار =====
    ...
    const updateRoleResponse = await request(server)
    ...
    // ===== اختبار 7: تعيين دور لمستخدم =====
    console.log('\n👤 تعيين دور لمستخدم...');

    // أولاً نحصل على معرف مستخدم
    const usersResponse = await request(server)
      .get('/roles/role_user/users') // الحصول على مستخدمين بدور المستخدم العادي
      .set('Authorization', `Bearer ${adminToken}`);
    ...
    if (testUserId) {
      const assignRoleResponse = await request(server)
        .post('/roles/assign')
        .set('Authorization', `Bearer ${adminToken}`)
        .send({
          userId: testUserId,
    ...
    // ===== اختبار 8: الحصول على إحصائيات الأدوار =====
    ...
    // ===== اختبار 9: حذف الدور (تنظيف) =====
    console.log('\n🗑️ حذف الدور (تنظيف)...');

    const deleteRoleResponse = await request(server)
