...
/**
 * سكريبت اختبار نظام إدارة المنتجات (Products Management)
 * يمكن تشغيله بـ: npm run products:test
 */
...
async function testProductsSystem() {
  ...
  try {
    ...
    const createCategoryResponse = await request(server)
      .post('/categories')
    ...
    // ===== اختبار 3: الحصول على جميع الفئات =====
    ...
    // ===== اختبار 5: الحصول على جميع المنتجات =====
    ...
    // ===== اختبار 6: البحث عن منتج بالباركود =====
    ...
    const createVariantResponse = await request(server)
    ...
    // ===== اختبار 8: الحصول على متغيرات المنتج =====
    ...
    const updateProductResponse = await request(server)
    ...
    const updateVariantResponse = await request(server)
    ...
    // ===== اختبار 11: إحصائيات المنتجات =====
    console.log('\n📊 إحصائيات المنتجات...');

    const productStatsResponse = await request(server)
      .get('/products/stats')
      .set('Authorization', `Bearer ${adminToken}`);
    ...
    // ===== اختبار 12: إحصائيات الفئات =====
    console.log('\n📈 إحصائيات الفئات...');

    const categoryStatsResponse = await request(server)
    ...
    // حذف متغير المنتج
    const deleteVariantResponse = await request(server)
    ...
    // حذف الفئة
    const deleteCategoryResponse = await request(server)
