    ...
    try {
        ...
        if (createUserResponse.status === 201) {
            ...
            if (usersResponse.status === 200) {
                const testUser = usersResponse.body.data.find((u) => u.username === 'audit_test_user');
                if (testUser) {
        ...
        if (statsResponse.status === 200 && statsResponse.body) {
            ...
            if (statsResponse.body.recentActivity && statsResponse.body.recentActivity.length > 0) {
                console.log('Recent activity (first 3):', statsResponse.body.recentActivity.slice(0, 3).map((activity) => `${activity.action} on ${activity.entity} by ${activity.userName}`));
            }
        }
        else {
        ...
        if (testProductId) {
            const trailResponse = await (0, supertest_1.default)(server)
            ...
            if (trailResponse.status === 200 && trailResponse.body) {
                ...
                if (trailResponse.body.changeHistory && trailResponse.body.changeHistory.length > 0) {
                    console.log('Change history (first 2):', trailResponse.body.changeHistory.slice(0, 2).map((change) => `${change.timestamp}: ${change.action} by ${change.user} (${change.changes.length} changes)`));
                }
            }
            else {
        ...
        const changesResponse = await (0, supertest_1.default)(server)
        ...
        if (changesResponse.status === 200 && changesResponse.body) {
            ...
            if (changesResponse.body.recentChanges && changesResponse.body.recentChanges.length > 0) {
                console.log('Recent changes (first 3):', changesResponse.body.recentChanges.slice(0, 3).map((change) => `${change.timestamp}: ${change.action} ${change.entity} by ${change.user}`));
            }
        }
        else {
        ...
        console.log('\n🔒 اختبار تقرير الأمان...');
        const securityResponse = await (0, supertest_1.default)(server)
        ...
        const complianceResponse = await (0, supertest_1.default)(server)
        ...
        if (testProductId) {
            ...
            if (updateResponse.status === 200) {
                ...
                const recentLogsResponse = await (0, supertest_1.default)(server)
