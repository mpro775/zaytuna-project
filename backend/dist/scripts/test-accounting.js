...
async function testAccountingSystem() {
    ...
    try {
        ...
        const setupResponse = await (0, supertest_1.default)(server)
        ...
        const glAccountsResponse = await (0, supertest_1.default)(server)
        ...
        if (glAccountsResponse.status === 200 && Array.isArray(glAccountsResponse.body)) {
            ...
            const cashAccount = glAccountsResponse.body.find((acc) => acc.accountCode === '1001');
            const salesRevenueAccount = glAccountsResponse.body.find((acc) => acc.accountCode === '4001');
            if (cashAccount)
            ...
            glAccountsResponse.body.slice(0, 3).forEach((account) => {
        ...
        console.log('\n🆕 إنشاء حساب GL مخصص...');
        const customAccountResponse = await (0, supertest_1.default)(server)
        ...
        const journalEntryResponse = await (0, supertest_1.default)(server)
        ...
        const journalEntriesResponse = await (0, supertest_1.default)(server)
        ...
        if (journalEntriesResponse.status === 200 && Array.isArray(journalEntriesResponse.body)) {
            ...
            journalEntriesResponse.body.slice(0, 2).forEach((entry) => {
        ...
        console.log('\n✅ اعتماد القيد اليومي...');
        const postResponse = await (0, supertest_1.default)(server)
        ...
        if (postResponse.status === 200) {
            ...
            const cashAccountResponse = await (0, supertest_1.default)(server)
                .get(`/accounting/gl-accounts/${cashAccountId}`)
                .set('Authorization', `Bearer ${adminToken}`);
            const salesAccountResponse = await (0, supertest_1.default)(server)
        ...
        const unpostResponse = await (0, supertest_1.default)(server)
        ...
        const autoSalesResponse = await (0, supertest_1.default)(server)
...
//# sourceMappingURL=test-accounting.js.map