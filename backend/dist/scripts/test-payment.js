...
async function testPaymentSystem() {
    ...
    try {
        ...
        const gatewaysResponse = await (0, supertest_1.default)(server)
        ...
        if (gatewaysResponse.status === 200 && gatewaysResponse.body) {
            ...
            console.log('Available gateways:', gatewaysResponse.body.gateways?.map((g) => g.name) || []);
        }
        else {
        ...
        if (testInvoiceId) {
            const localPaymentResponse = await (0, supertest_1.default)(server)
        ...
        const currencySupportResponse = await (0, supertest_1.default)(server)
        ...
        const methodSupportResponse = await (0, supertest_1.default)(server)
        ...
        const gatewaysForCurrencyResponse = await (0, supertest_1.default)(server)
        ...
        if (gatewaysForCurrencyResponse.status === 200 && gatewaysForCurrencyResponse.body) {
            ...
            console.log('Gateways for SAR:', gatewaysForCurrencyResponse.body.gateways?.map((g) => g.name) || []);
        }
        console.log('\n💳 البوابات المتاحة لطريقة دفع محددة...');
        const gatewaysForMethodResponse = await (0, supertest_1.default)(server)
        ...
        if (gatewaysForMethodResponse.status === 200 && gatewaysForMethodResponse.body) {
            ...
            console.log('Gateways for card payments:', gatewaysForMethodResponse.body.gateways?.map((g) => g.name) || []);
        }
        console.log('\n⚖️ اختبار تسوية المعاملات...');
        const reconciliationResponse = await (0, supertest_1.default)(server)
        ...
        console.log('\n📈 اختبار تقرير الدفع...');
        const reportResponse = await (0, supertest_1.default)(server)
        ...
        const performanceResponse = await (0, supertest_1.default)(server)
