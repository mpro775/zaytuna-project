...
let stockItems = getMockDataFromStorage('stockItems', stockItemsData);
let stockMovements = getMockDataFromStorage('stockMovements', stockMovementsData);

// Register handlers
mockApi.registerHandler('GET:/inventory/stock-items', async (request: MockRequest): Promise<MockResponse> => {
...
mockApi.registerHandler('GET:/inventory/stock-items/:id', async (request: MockRequest): Promise<MockResponse> => {
...
mockApi.registerHandler('POST:/inventory/stock-items', async (request: MockRequest): Promise<MockResponse> => {
  const newItem = {
    id: generateId(),
    ...request.data,
...
mockApi.registerHandler('PATCH:/inventory/stock-items/:id', async (request: MockRequest): Promise<MockResponse> => {
  const id = request.params?.id;
  ...
  if (stockItems[index].quantity < stockItems[index].minStock) {
    stockItems[index].isLowStock = true;
  } else {
    stockItems[index].isLowStock = false;
  }
  
  if (stockItems[index].quantity > stockItems[index].maxStock) {
    stockItems[index].isOverStock = true;
  } else {
    stockItems[index].isOverStock = false;
  }
...
mockApi.registerHandler('POST:/inventory/stock-items/:warehouseId/:productVariantId/adjust', async (request: MockRequest): Promise<MockResponse> => {
  ...
  const movement = {
    ...
    quantity: request.data.quantity || 0,
    referenceType: request.data.referenceType,
...
mockApi.registerHandler('GET:/inventory/movements', async (request: MockRequest): Promise<MockResponse> => {
...
mockApi.registerHandler('GET:/inventory/alerts/low-stock', async (request: MockRequest): Promise<MockResponse> => {
...
mockApi.registerHandler('GET:/inventory/stats', async (request: MockRequest): Promise<MockResponse> => {
...
mockApi.registerHandler('GET:/inventory/products/:productVariantId/stock', async (request: MockRequest): Promise<MockResponse> => {
  const productVariantId = request.params?.productVariantId;
...
mockApi.registerHandler('GET:/inventory/warehouses/:warehouseId/stock', async (request: MockRequest): Promise<MockResponse> => {
  const warehouseId = request.params?.warehouseId;
...
mockApi.registerHandler('POST:/warehouses/transfer-stock', async (request: MockRequest): Promise<MockResponse> => {
  const { fromWarehouseId, toWarehouseId, productVariantId, quantity, notes } = request.data;
  
  // Find source item
  let sourceItem = stockItems.find(
  ...
  const inMovement = {
    ...
    quantity: quantity,
    referenceType: 'warehouse_transfer',
    referenceId: outMovement.referenceId,
    reason: notes || 'نقل بين المخازن',
    performedBy: 'user-5',
    createdAt: new Date().toISOString(),
  };
